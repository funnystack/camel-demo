...
//	var width=(window.innerWidth-100||800);
//	var height=(window.innerHeight-100||500);
	...
	if(property.haveHead){
		...
		//添加隐藏、显示
		tmp+="<a href='javascript:void(0)' class='GooFlow_head_btn'><i class='ico_hide'></i></a>"
		...
		this.onMenuHide=onMenuHide;//菜单隐藏
		this.onMenuShow=onMenuShow;//菜单显示
		this.onHome=onHome;//回到主页
		this.onReload=onReload;//刷新
		this.onFullScreen=onFullScreen;//全屏
		...
		this.$head.on("click",{inthis:this},function(e){
			...
			switch($(tar).attr("class")){
				...
				case "ico_save":	if(This.onReload!=null)	This.onReload();break;
				case "ico_undo":	This.undo();break;
				case "ico_redo":	This.redo();break;
				case "ico_reload":if(This.onReload!=null)	This.onReload();break;
				//隐藏菜单
				case "ico_hide":if(This.onMenuHide != null) This.onMenuHide();break;
				//显示菜单
				case "ico_show":if(This.onMenuShow != null) This.onMenuShow();break;
				case "ico_home":if(This.onHome != null) This.onHome();break;
				case "ico_fullScreen":if(This.onFullScreen != null) This.onFullScreen();break;
				case "ico_quitFullScreen":if(This.onFullScreen != null) This.onFullScreen();break;
			}
		});
	}
	var toolWidth=0;
	if(property.haveTool){
		this.$bgDiv.append("<div class='GooFlow_tool'"+(property.haveHead? "":" style='margin-top:4px'")+"><div class='GooFlow_tool_div'></div></div>");
...
//			}
			for(var i=0;i<property.consumerComponent.length;++i){
				tmp+="<a href='javascript:void(0)' type='"+property.consumerComponent[i]+"' id='"+this.$id+"_btn_"+property.consumerComponent[i].split(" ")[0]+"' class='GooFlow_tool_btn'><i class='ico_"+property.consumerComponent[i]+"'/></a>";//加入自定义按钮
			}
			//加入区域划分框工具开关按钮
			if(property.haveGroup)
				this.$tool.append("<a href='javascript:void(0)' type='group' class='GooFlow_tool_btn' id='"+this.$id+"_btn_group'><i class='ico_group'/></a>");
			
			//添加生产者组件
			tmp += "<span/>";
			for(var i=0;i<property.producerComponent.length;++i){
				tmp+="<a href='javascript:void(0)' type='"+property.producerComponent[i]+"' id='"+this.$id+"_btn_"+property.producerComponent[i].split(" ")[0]+"' class='GooFlow_tool_btn'><i class='ico_"+property.producerComponent[i]+"'/></a>";//加入自定义按钮
			}
	...
	this.$bgDiv.append("<div class='GooFlow_work' style='width:"+(width+10)+"px;height:"+(height-50)+"px;"+(property.haveHead? "":"margin-top:3px")+"'></div>");
	...
	if(this.$editable){
	  ...
	  this.onItemRename=onItemRename;
	  ...
	  this.onLinePointMove=onLinePointMove;
...
GooFlow.prototype={
	...
	addNode:function(id,json){
      ...
      if(json.type.indexOf(" mix")>-1){
        ...
        var nodeColor = GooFlow.prototype.color.node;
        if(typeof(GooFlow.prototype.color[json.type]) != 'undefined'){
			   nodeColor = GooFlow.prototype.color[json.type];
        }
      
       	this.$nodeDom[id].css({"background-color":nodeColor,"border-color":GooFlow.prototype.color.node});
      }
...
///////////以下为有关画线的方法
	...
	delLine:function(id,trigger){
		if(!this.$lineData[id])	return;
		if(false!=trigger && this.onItemDel!=null&&!this.onItemDel(id,"line"))	return;
		if(this.$undoStack){
	...
	delArea:function(id, trigger){
		...
		if(false!=trigger && this.onItemDel!=null&&!this.onItemDel(id,"area"))	return;
...
//		this.$bgDiv.css({height:h+"px",width:w+"px"});
		this.$bgDiv.css({height:"93%",width:"100%"});
...
//		this.$workArea.parent().css({height:h+"px",width:w+"px"});
		this.$workArea.parent().css({height:"90%",width:"100%"});
...
//		this.$workArea.css({height:h+"px",width:w+"px"});
		this.$workArea.css({height:"90%",width:"100%"});
...
//		this.$draw.style.height = +h + "px";
		...
		this.$draw.style.height = "90%";
		
		if(this.$group!=null){
//			this.$group.css({height:h+"px",width:w+"px"});
			this.$group.css({height:"90%",width:"100%"});
		}
		$(".GooFlow_work_inner").css({width:"100%",height:"90%"});
		log($(".ico_hide"));
		if(hideFlag){
			$(".GooFlow_work").css({width:"100%",height:"100%"});
		}else{
			$(".GooFlow_work").css({width:"100%",height:"90%"});
		}
	}
}
