...
import com.funny.admin.camelv.constant.LogConstant;
...
public class BeforeEndpoint extends ResourceEndpoint {
	...
	protected void onExchange(Exchange exchange) throws Exception {
		...
		if (RouteType.ROUTE_TYPE_JETTY.equals(type)) {
			/**
			 * jetty入口触发，记录调用的一些基本信息<br/>
			 */
			startJetty(exchange, route);
	...
	private void permissionValidation(Exchange exchange, CamelvRoute route) throws Exception {
		try {
			String permissionValidationSwitch = Global.getConfig("camelv.permission.validation.switch");
			if (!"true".equals(permissionValidationSwitch)) {
				/** 不做权限验证 */
				return;
			}
			String permissionHeader = Global.getConfig("camelv.permission.header.name");
	...
	private void startJetty(Exchange exchange, CamelvRoute route) throws Exception {
		// 是否开启持久化
		String persistSwitch = Global.getConfig("camelv.persist.switch");
		String oldBody = exchange.getIn().getBody(String.class);
		if ("true".equals(persistSwitch)) {
			/** 开启持久化 */
			CamelvServer server = CamelvContext.getCamelvServer(route.getServerId());
			if (server == null) {
				exchange.setProperty(Constant.ROUTE_STATE, RouteState.SERVER_DELETED);
				throw new Exception("服务已经被删除");
			}
			exchange.setProperty("serverId", server.getId());
			logger.info("入口报文:\n" + oldBody);
			// 记录调用信息
	...
	private void beforeRunHttp(Exchange exchange, CamelvRoute route) throws Exception {
		...
		// 资源找到了，那么就需要执行
		exchange.setProperty(Constant.NEXT_URI, camelvHttp.getUrl());
		logger.info("下一个路由地址:" + camelvHttp.getUrl());
	}
