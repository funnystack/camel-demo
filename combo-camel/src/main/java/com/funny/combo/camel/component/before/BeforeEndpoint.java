...
public class BeforeEndpoint extends ResourceEndpoint {
    ...
    protected void onExchange(Exchange exchange) throws Exception {
        ...
        if (RouteTypeEnum.ROUTE_TYPE_RPC.getType().equals(type)) {
            beforeRunHttp(exchange, route);
        }
        else if (RouteTypeEnum.ROUTE_TYPE_GROOVY.getType().equals(type)) {
            beforeRunGroovy(exchange, route);
        }
        else if (RouteTypeEnum.ROUTE_TYPE_BEAN.getType().equals(type)) {
    ...
    private void beforeRunHttp(Exchange exchange, CamelvRoute route) throws Exception {
        ...
        // 获取关联的http信息
        CamelvHttp camelvHttp = CamelvContext.getCamelvHttp(route.getRouteId());
        if (camelvHttp == null) {
