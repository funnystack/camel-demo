...
import com.xiaoka.camelv.util.CamelvHttpUtils;
...
public class HttpsEndpoint extends ResourceEndpoint {
	...
	protected void onExchange(Exchange exchange) {
		...
		if (Constant.HTTP_GET.equals(requestType)) {
			resultBody = CamelvHttpUtils.sendHttpsGet(url, headerMap);
			...
			resultBody = CamelvHttpUtils.sendHttpsPost(url, contentType, headerMap, oldBody);
		}
		// 设置结果到body中
