...
//初始化区域图的对象
	...
	if(property.haveTool){
		this.$bgDiv.append("<div class='GooFlow_tool'"+(property.haveHead? "":" style='margin-top:3px'")+"><div class='GooFlow_tool_div'></div></div>");
		...
		if(property.haveGroup){
            ...
            this.$tool.append("<a href='javascript:void(0)'"+titleGroup+" type='group' class='GooFlow_tool_btn' id='"+this.$id+"_btn_group'><i class='ico_group'/></a>");
		}
		toolWidth=31;
		this.$nowType="cursor";
		//绑定各个按钮的点击事件
...
//	this.$bgDiv.append("<div class='GooFlow_work' style='"+(property.haveHead? "top:5px;":"")+(property.haveTool? "left:2px":"")+"'></div>");
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".ico","mousedown",{inthis:this},function(e){
			...
			document.onmousemove=function(e){
				...
				This.$ghost.css({left:X-40+"px",top:Y+50+"px"});
				isMove=true;
			}
			document.onmouseup=function(e){
...
///////////以下为有关画线的方法
	...
	setLineType:function(id,newType,M){
		log("setLineType id = "+id+" , newType = "+newType+" , M = "+M);
	...
	reinitSize:function(width,height){
		...
		this.$bgDiv.css({height:h+"px",width:"100%"});
		...
		this.$draw.style.width = w+10 + "px";
		this.$draw.style.height = h + "px";
		
		//设置GooFlow_work大小
		log("GooFlow_work :"+h+" , "+w);
		$(".GooFlow_work").css({width:width+10+"px",overflow:"hidden"});
